...
std::pair<std::vector<std::pair<double, double>>, std::vector<std::pair<double, double>>> Graph::H2H_way(int s,int t,int dist){
    int i=0;
    ...
    //std::cout<<"T"<<std::endl;
    for(auto poin:le)path.push_back({lat[poin],lon[poin]});
    path.push_back({lat[u],lon[u]});
    for(i=ri.size()-1;i>=0;i--)path.push_back({lat[ri[i]],lon[ri[i]]});
    return {path,search};
};
